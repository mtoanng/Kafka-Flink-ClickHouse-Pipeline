...
Đây là **đồ án môn Android (Phát triển ứng dụng di động)** — **độc lập** với đồ án môn Java của Leader.

- **Team Android**: chỉ có **2 người** — Leader (mình) + bạn (Android Developer).
- **Repo**: bạn sẽ tạo **repo Android riêng** (mới hoàn toàn, không clone vào repo Java).
- **Tương quan với đồ án Java**: chỉ là **REST API consumer** — đồ án Java đã có sẵn backend Spring Boot expose 14 endpoint, bạn consume qua HTTP. Không cần biết Kafka/Flink/Pillar internals.
...
Bạn là **Android Owner** — chịu 100% mảng mobile, gồm:
...
## 4. Tech stack — Khuyến nghị cho intermediate dev

> 🎯 **Triết lý**: đồ án sinh viên, ưu tiên **đơn giản + đủ điểm**. Đừng over-engineer.
...
## 5. API Contract — 14 endpoint bạn sẽ consume
...
Trong 1-2 tuần đầu khi Leader chưa build Phase 4.5 (proxy block), bạn có 2 option:

**Option A — Mock server bằng `json-server`** (5 phút setup):
```bash
npx json-server --watch mock-api.json --port 8090
```
Leader sẽ cung cấp file `mock-api.json` chứa response mẫu cho 14 endpoint.
