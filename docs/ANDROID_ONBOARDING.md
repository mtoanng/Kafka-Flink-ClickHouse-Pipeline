...
Đây là **đồ án môn Android (Phát triển ứng dụng di động)** — đã có sẵn backend Spring Boot expose 13 canonical endpoint (mỗi pillar còn 1-2 legacy alias để backward-compat), chỉ cần consume qua HTTP. Không cần biết Kafka/Flink/Pillar internals.
...
**Leader** sẽ:
- Cung cấp **REST API endpoint** (URL + 13 canonical endpoint sẵn sàng)
...
## 5. API Contract — 13 canonical endpoint sẽ được consume

> ⚠️ **Onboarding doc snapshot** (Phase 2.5/2.6 baseline). Tham khảo nhanh OK, **nhưng để biết
> contract hiện tại** (Phase 7.1 IEA/APERC + Phase 7.6 backend migration) hãy đọc:
> - [`docs/openapi.json`](./openapi.json) — OpenAPI 3 spec (20 paths = 13 canonical + 6 legacy aliases + 1 acknowledge sub-path)
> - [`docs/VES-Monitor.postman_collection.json`](./VES-Monitor.postman_collection.json) — 8 folders / 19 requests
> - `backend-api/src/main/java/vn/edu/ves/api/dto/*Dto.java` — actual Lombok DTOs returned by controllers
>
> Một vài chỗ khác biệt so với baseline mục 5 bên dưới:
> - Pillar canonical paths bây giờ là `/api/pillars/{1..4}/{supply-security,market-resilience,grid-reliability,energy-transition}` — phần legacy `/outlook,/volatility,/shedding,/netzero` còn giữ làm alias.
> - Pillar DTOs đổi shape theo 16 sub-indicator IEA/APERC (IDR, SFRI, HHI, N-1, σ30d, price_gap, β_crude, affordability, reserve_margin, peak_factor, shed_prob, freq_stability, renewable_pct, CO2_intensity, curtailment, netzero_progress) — **không còn** `stockDays/targetDays/deficitDays/supplyStatus`.
> - Status enum hiện tại: `SECURE | ELEVATED | STRESSED | CRITICAL` (không còn `STABLE/AT_RISK`); timestamp field tên `computedAt` (không phải `calculatedAt`); không có field `trend`.
> - Raw data paths đã bỏ prefix `/raw/`: `/api/fuel-prices/latest` và `/api/grid-load/latest`.
> - `LoginResponse` chỉ có `accessToken / expiresInMs (ms) / user` — không có `tokenType` / `expiresIn (s)`.
> - Acknowledge request body có cả `status` (`ACKNOWLEDGED` | `DISMISSED`) **và** `note` (optional); response trả `{id, newStatus, acknowledgedBy}`.
> - Android sibling repo (`mtoanng/DataStream`) cuối cùng dùng **Moshi** (không phải Gson trong stack table mục 4).
...
### 5.3 Bảng endpoint (baseline Phase 2.5/2.6 — xem callout đầu mục 5 cho contract hiện tại)
