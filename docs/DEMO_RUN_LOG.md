...
> **✅ Phase 7.6 update (13 May 2026, ~04:30 ICT):** the regression flagged above has been **fixed**. `PillarDao` / `SecurityDao` and 4 DTOs were migrated to the new IEA/APERC view shape, `PillarController` now exposes BOTH legacy paths (`/1/outlook`, `/2/volatility`, `/3/shedding`, `/4/netzero`) AND new paths (`/1/supply-security`, `/2/market-resilience`, `/3/grid-reliability`, `/4/energy-transition`) — same handler, backward-compat. `/api/security/cascade-risks` returns HTTP 200 with an empty array (`@Deprecated` — `v_cascade_risks` was dropped in Phase 7.1; cascade analysis to be re-implemented post-Phase 8). **Re-smoke result: 13 / 13 endpoints `200 OK`** (`build-logs/p76-api-smoke.json`). 11 new `@WebMvcTest` smoke tests pass (`build-logs/p76-backend-test.log`). Backend re-packaged (30.30 MB), restarted on port 8090. Phase 7 backend status flipped `⚠ → ✅`.
