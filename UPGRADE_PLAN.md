...
| **Tính năng từng màn (đã implement)** | **Login**: form + validate (NotBlank) + BCrypt verify với 3 seed users từ Phase 2 + lưu SessionManager.AtomicReference. **Dashboard**: top bar ProgressIndicator gauge bind overallScore/100 + lblStatus color-coded SECURE/STABLE/AT_RISK/CRITICAL + TabPane 4 tab — Pillar 1: TableView `v_pillar1_supply_outlook` (cột recommendation_text) + BarChart stock_days/region. Pillar 2: TableView `v_pillar2_volatility_signal` + LineChart avg price. Pillar 3: TableView `v_pillar3_load_shedding_plan` + BarChart bars colored theo load_pct (≥95 red, ≥85 orange, ≥70 yellow). Pillar 4: TableView `v_pillar4_net_zero_progress` + PieChart share %. **Right sidebar**: ListView `v_active_recommendations` severity-styled. **Auto-refresh 30s** ScheduledExecutorService + Task background. **Region CRUD**: form (code/name/country/vn_zone/description) + TableView, validator chain compose (NotBlank + LengthRange + Pattern HOA + InSet). **AlertRule CRUD**: ChoiceBox multi-pillar MetricType + Operator + Severity enum, threshold numeric pattern validator, toggle enable button. **User CRUD**: ADMIN-only enforced trong controller initialize + service delete check, optional password change, self-delete guard. |
| **Design patterns đã triển khai (môn Java checkpoint)** | Singleton (`DatabaseConfig`, `SessionManager` với AtomicReference), DAO (4 DAO extends BaseDao), MVC (FXMLLoader + 6 Controller + 13 Model), **Strategy + Composite** (`util/Validator` interface + 4 strategy implementations + `compose(...)` collect all errors), Factory (`DatabaseConfig.openConnection()` trả Connection mới mỗi call). |
| **Test coverage (62 @Test)** | DAO 24 (UserDao 7 + RegionDao 6 + AlertRuleDao 5 + ViewsDao 6) qua H2 MODE=PostgreSQL in-memory. Service 22 (Auth 6 + Region 4 + AlertRule 3 + User 5 + Dashboard 4) qua Mockito mock DAO. Util 16 (PasswordUtil 5 với real BCrypt + SessionManager 6 + Validator 5). |
| **Smoke test plan** | `mvn -pl desktop-admin test` → expect 62/62 PASS. `mvn -pl desktop-admin javafx:run` → cửa sổ login → admin/admin → Dashboard 4 tab + recommendation sidebar. Click nav Regions → CRUD region form load 6 seed row. Click Users → cảnh báo nếu non-admin. |
| **Common pitfalls (đã handle sẵn)** | (1) JavaFX 21 yêu cầu Java 17+ → chọn 17.0.10 LTS giữ Java 11. (2) `module-info.java` overhead → classpath mode (javafx-maven-plugin tự xử lý). (3) `suggested_data` JSONB H2 không hiểu → dùng MODE=PostgreSQL trong H2 URL để `::text` cast hoạt động. (4) DatabaseConfig singleton final fields → reflection override `setAccessible(true)` trong test (Java 11 default JVM cho phép). (5) Dashboard auto-refresh thread blocking UI → ScheduledExecutorService daemon + Task background + Platform.runLater apply snapshot. (6) Self-delete admin → guard cả controller (UX) lẫn service (defense in depth). |
| **Tag** | `v0.5-javafx-desktop` (sẽ tag sau khi build + smoke test xanh) |
| **Tài liệu** | [`desktop-admin/README.md`](./desktop-admin/README.md) — directory tree, design pattern table, test count từng file, override DB env. [`docs/PROGRESS.md`](./docs/PROGRESS.md) — log Phase 5.1 → 5.5 detail. |
...
| **7.5** Final QA pass | ✅ | `30265f1` | Re-verified full stack on `c7ef8bd`. 77/77 desktop tests, lint clean, 5/5 docker containers, Flink 18/18 vertices, ESI=72.82 ELEVATED. **Backend regression flagged**: 5 `/api/pillars/*` + cascade-risks endpoints `500` because Phase 7.1 dropped legacy views without migrating `PillarDao`/`SecurityDao`. Documented; deferred to Phase 7.6. |
| **7.6** Backend pillar migration | ✅ | (this commit) | Closes Phase 7.5 regression. `PillarDao` + `SecurityDao` rewired to new `v_pillar*_supply_security/market_resilience/grid_reliability/energy_transition` views; 4 DTOs renamed (`Pillar{1..4}{SupplySecurity,MarketResilience,GridReliability,EnergyTransition}Dto`); `PillarController` exposes BOTH legacy + new paths for backward-compat; `/api/security/cascade-risks` returns 200 + empty array (`@Deprecated`, view dropped Phase 7.1). 11 new `@WebMvcTest` smoke (JUnit 4 + SpringRunner). `docs/openapi.json` (20 paths) + `docs/VES-Monitor.postman_collection.json` (8 folders / 19 requests / auto-token script). **REST smoke: 13/13 `200 OK`** (`build-logs/p76-api-smoke.json`). JAR re-package 30.30 MB. **Phase 7 backend status: ✅**. |
